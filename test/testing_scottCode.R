...
#### USER INPUT ####
# Load mapping file
MAP <- readRDS("./data/metadata/combinedMetadata.RDS")
rownames(MAP) <- MAP$sample_names
MAP <- sample_data(MAP)
# Load viral taxonomy and remove any non-viral sequences
viral_table <- fread(file = "./data/hecatomb_out/viruses_tax_table.tsv", header = TRUE, stringsAsFactors=TRUE)
setkey(viral_table, Kingdom)
viral_table <- viral_table[!"Bacteria"]

# Import and adjust count table
seqtable <- fread(file = "./data/hecatomb_out/seqtable.all", header = TRUE, sep = "\t", drop = 2)
seqtable <- setnafill(seqtable, fill=0)

# Import alignment files
# aa_checked alignment
aln.aa <- fread(file = "./data/hecatomb_out/aa.aln.m8", header = TRUE, sep = "\t")
aln.aa <- aln.aa %>%
  rename(id = query)

# nt_checked alignment
aln.nt <- fread(file = "./data/hecatomb_out/nt.aln.m8", header = TRUE, sep = "\t")
aln.nt <- aln.nt %>%
  rename(id = query)
...
#### USER INPUT ####
names(library.size) <- c("sample_names", "library_size") #library.size %>%
  #rename(sample_names = value) %>% # Rename to whatever your sequence file name / sample name is in your mapping file
...
baltimore <- fread("./data/descriptions/2020_07_27_Viral_classification_table_ICTV2019.txt", stringsAsFactors = TRUE)
...
ps.melt.fixed.sp <- merge(ps.melt.fixed.sp, library.size, all.x = TRUE, by = "sample_names")
...
# Merge with library size
ps.melt.fixed.ge <- merge(ps.melt.fixed.ge, library.size, all.x = TRUE, by = "sample_names")
...
otutab <- otu_table(ps.aln)
taxa_are_rows(otutab)
testMeltOTU <- reshape2::melt(as(otutab, "matrix"))
colnames(testMeltOTU) <- c("OTU", "Sample", "Abundance")

comparePS <- select(testMeltOTU, Sample, Abundance)
compareMe <- mergedPivot %>% 
  select(Sample, Abundance)

all_equal(comparePS, compareMe)



mtx <- as(otutab, "matrix")
...
descriptions <- fread("./data/descriptions/uniprot.description.db", stringsAsFactors = TRUE, col.names = c("target", "protein"))
...
# Merge with library size
ps.aln.fixed <- merge(ps.aln.fixed, library.size, all.x = TRUE, by = "sample_names")
...
saveRDS(ps.aln.scaled, file = "ps.aln.scaled.RDS")

# Write zero abundance filtered file
ps.aln.fixed.filt <- ps.aln.scaled %>%
  filter(Abundance > 0)
saveRDS(ps.aln.fixed.filt, file = "ps.aln.fixed.filt.RDS")
